...
class Order {
 ...
 public static function updateCart(int $customerId,int $productId,int $qty): void { $pdo=Database::getConnection(); if($qty<1){ $s=$pdo->prepare('DELETE FROM carts WHERE customer_id=:c AND product_id=:p'); $s->execute(['c'=>$customerId,'p'=>$productId]); } else { $q=max(1,min(99,$qty)); $s=$pdo->prepare('INSERT INTO carts(customer_id,product_id,quantity) VALUES(:c,:p,:q) ON DUPLICATE KEY UPDATE quantity=VALUES(quantity)'); $s->execute(['c'=>$customerId,'p'=>$productId,'q'=>$q]); } }
