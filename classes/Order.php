...
class Order {
 ...
 public static function place(int $customerId,string $note='', ?string $couponCode = null): int { $pdo=Database::getConnection(); $items=self::cart($customerId); if(!$items) throw new RuntimeException('سبد خرید خالی است.'); $pdo->beginTransaction(); try{ $total=0; foreach($items as $i) $total+=(int)$i['price']*(int)$i['quantity']; $appliedCouponCode = null; $appliedCouponPercent = null; $discountAmount = 0; if ($couponCode && trim($couponCode) !== '') { require_once __DIR__ . '/Coupon.php'; $c = Coupon::findByCode(trim($couponCode)); if (!$c || !Coupon::isValidCoupon($c)) { throw new RuntimeException('کد کوپن نامعتبر یا منقضی شده است.'); } $appliedCouponCode = $c['code']; $appliedCouponPercent = (int)$c['percent']; $discountAmount = (int) round($total * $appliedCouponPercent / 100); } $finalTotal = max(0, $total - $discountAmount); $number='DNJ-'.date('Ymd').'-'.strtoupper(bin2hex(random_bytes(3))); $s=$pdo->prepare('INSERT INTO orders(customer_id,order_number,total_price,customer_note,coupon_code,coupon_percent,discount_amount) VALUES(:c,:n,:t,:note,:ccode,:cpercent,:damount)'); $s->execute(['c'=>$customerId,'n'=>$number,'t'=>$finalTotal,'note'=>mb_substr(trim($note),0,500),'ccode'=>$appliedCouponCode,'cpercent'=>$appliedCouponPercent,'damount'=>$discountAmount]); $id=(int)$pdo->lastInsertId(); $ins=$pdo->prepare('INSERT INTO order_items(order_id,product_id,product_name,quantity,price) VALUES(:o,:p,:n,:q,:price)'); foreach($items as $i) $ins->execute(['o'=>$id,'p'=>$i['product_id'],'n'=>$i['name'],'q'=>$i['quantity'],'price'=>$i['price']]); $pdo->prepare('DELETE FROM carts WHERE customer_id=:c')->execute(['c'=>$customerId]); $pdo->commit(); ActivityLog::record('order_create','customer',$customerId,$_SESSION['customer_display_name']??null,$id,'ثبت سفارش '.$number); return $id; }catch(Throwable $e){ if($pdo->inTransaction())$pdo->rollBack(); throw $e; } }
