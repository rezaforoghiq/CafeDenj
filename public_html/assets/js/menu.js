document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('[data-cart-add]').forEach(form => form.addEventListener('submit', async event => { event.preventDefault(); const button = form.querySelector('button'); button.disabled = true; try { const response = await fetch(form.action, { method: 'POST', body: new FormData(form) }); const data = await response.json(); if (!data.success) throw new Error(data.message || 'خطا در افزودن محصول'); const count = Number(data.count) || 0; document.querySelectorAll('[data-cart-count]').forEach(badge => { badge.textContent = count ? count.toLocaleString('fa-IR') : ''; badge.classList.toggle('is-empty', !count); }); document.querySelectorAll('.header-cart-btn').forEach(btn => { btn.classList.toggle('is-hidden', !count); }); document.querySelectorAll('.header-cart-badge').forEach(badge => { badge.classList.remove('pulse'); void badge.offsetWidth; badge.classList.add('pulse'); }); button.textContent = '✓ به سبد اضافه شد'; button.classList.add('added'); let toast = document.querySelector('.cart-toast'); if (!toast) { toast = document.createElement('div'); toast.className = 'cart-toast'; document.body.append(toast); } toast.textContent = 'محصول به سبد خرید اضافه شد · ' + (count ? count.toLocaleString('fa-IR') : data.count) + ' محصول در سبد'; requestAnimationFrame(() => toast.classList.add('show')); setTimeout(() => { toast.classList.remove('show'); button.textContent = 'افزودن به سبد'; button.classList.remove('added'); }, 1800); } catch (error) { alert(error.message); } finally { button.disabled = false; } }));
