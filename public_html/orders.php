...
$labels = ['pending'=>'در انتظار تأیید','approved'=>'تأیید شده و در حال آماده‌سازی','rejected'=>'رد شده','completed'=>'تکمیل و تحویل داده شده'];

if (isset($_GET['poll']) && $_GET['poll'] === '1') {
    header('Content-Type: application/json; charset=utf-8');
    $statuses = array_map(function($o) use ($labels) {
        return [
            'id' => (int)$o['id'],
            ...
            'can_delete' => $o['status'] === 'pending',
        ];
    }, $orders);
    echo json_encode(['success' => true, 'count' => count($statuses), 'statuses' => $statuses], JSON_UNESCAPED_UNICODE);
    exit;
}
?>
...
<section class="orders-list"><?php foreach ($orders as $order): ?><article class="order-card" data-order-id="<?= (int) $order['id'] ?>"><div class="order-card-top"><div><span class="order-number">سفارش شماره <?= htmlspecialchars($order['order_number'], ENT_QUOTES, 'UTF-8') ?></span><span class="order-date"><?= htmlspecialchars($order['created_at'], ENT_QUOTES, 'UTF-8') ?></span></div><span class="order-status status-<?= htmlspecialchars($order['status'], ENT_QUOTES, 'UTF-8') ?>" data-status="<?= htmlspecialchars($order['status'], ENT_QUOTES, 'UTF-8') ?>"><?= $labels[$order['status']] ?? 'نامشخص' ?></span></div><div class="order-card-bottom"><b><?= number_format((float) $order['total_price']) ?> <small>تومان</small></b>
