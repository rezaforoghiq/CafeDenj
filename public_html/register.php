...
$errors = [];
$old = ['phone' => '', 'first_name' => '', 'last_name' => ''];
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? null)) {
        ...
        try {
            ...
            $validationErrors = CustomerAuth::validateRegistration($phone, $old['first_name'], $old['last_name']);
            if ($validationErrors !== []) {
                $errors = array_merge($errors, $validationErrors);
            } else {
                $pendingId = PendingRegistration::create(Database::getConnection(), $phone, $old['first_name'], $old['last_name'], '');
...
<body class="customer-auth">
    <main class="auth-card">
        <div class="auth-brand">
            ...
            <p>ورود و ثبت‌ نام سریع و امن با کد یک‌بار مصرف پیامکی</p>
        </div><?php if (isset($errors['general'])): ?><div class="auth-alert"><?= htmlspecialchars($errors['general'], ENT_QUOTES, 'UTF-8') ?></div><?php endif; ?><form method="post" data-auth-form novalidate><input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrfToken(), ENT_QUOTES, 'UTF-8') ?>">
            ...
            <button class="auth-submit" type="submit">دریافت کد تأیید و ثبت‌نام</button>
        </form>
        <p class="auth-footer">حساب دارید؟ <a href="login">وارد شوید</a></p><a class="back-home" href="index">بازگشت به منو</a>
    </main>
