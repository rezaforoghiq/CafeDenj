    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? null)) {
        ...
        try {
            ...
            if ($account === null) {
                ...
                $otpService->issueOtp($identity, 'login', $_SERVER['REMOTE_ADDR'] ?? null);
                $_SESSION['otp_context'] = [
                    'purpose' => 'login',
                    'phone' => CustomerAuth::normalizePhone($identity),
                    'account_id' => (int) $account['account_id'],
                    'customer_id' => (int) $account['customer_id'],
                ];
                $_SESSION['otp_notice'] = 'کد تأیید برای ورود به حساب شما ارسال شد.';
                header('Location: otp');
                exit;
            }
